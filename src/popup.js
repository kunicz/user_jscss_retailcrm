...
function create(meta) {
	const $popup = $(`
		<div id="custom_popup" data-page="${meta.id}" aria-hidden="false" aria-modal="true" role="dialog" class="popup_VIxGl omnica-modal">
	...
	`);
	$popup.appendTo('body');
	$popup.find('.omnica-modal-window__close').on('click', () => $popup.hide());
}

function update($popup, meta) {
	$popup.data('page', meta.id);
	$popup.find('#custom_popup_title').text(meta.title);
	$popup.show();
}

export function searchForm(target, placeholder) {
